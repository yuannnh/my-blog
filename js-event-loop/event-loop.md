...
- 线程 (Thread)：操作系统执行和调度的最小单元，它是也是一个程序进行自我分割然后并行执行的最小单元。（来自并概括自[维基百科](https://en.wikipedia.org/wiki/Thread_(computing))）
...
- Web API 在这里是有浏览器提供的一些**外部API** (如果你在Node 环境中，那就会是由Node提供的一些API)，它们常常是**异步调用**的。
- 它们会接受**一个参数叫“回调函数”(Callback)**, 这个函数会在API触发某个事件时执行。
- 当Stack顶端的函数调用API时，API 的内容就会在另外的线程中执行（原来所谓的并行还是用到了多个线程）。API在监听到某些事件时会把对应的回调函数放到一个**回调函数队列**里。这个事件可以是一个http请求的结束，一个 Timer倒计时结束，或者一个鼠标点击事件。

#### 回调函数队列 (Callback queue)

- 回调函数队列是一个FIFO-first in first out（先进先出）的数据结构。它放入顺序存储了需要执行的回调函数。
- 当 Stack 空了的时候，它就会把队列中第一个回调函数发送到 Stack 里去调用。
...
3. 执行func1 第二行：调用一个计时API (在0秒后将callback1放入回调函数队列)

4. 

   1. func2被放入stack，并开始执行func2的内容。

   2. 由于上一步中的延迟是0秒，所以callback1立刻会被放入回调函数队列

      以上两步可以理解为是同时发生的因为他们在两个不同的线程里执行

5. 执行func2 第一行："func2 开始"

6. 执行func2 第二行：调用一个计时API (在1秒后将callback1放入回调函数队列)

7. 执行func2 第三行：打印"func2 结束" 

8. 将func2 的调用从stack中删除

9. 执行func1 第三行：打印"func1 结束" 

10. 将func1 的调用从stack中删除

11. 事件循环机制发现stack空了，发消息给回调函数队列

12. callback1被放入stack执行

13. 执行callback1第一行：打印 "我是 callback1"

14. 将callback1的调用从stack中删除

15. 事件循环机制发现stack空了，发消息给回调函数队列，但队列中什么也没有，所以什么也不做

16. 1秒时间到，callback2被放入回调函数队列

17. 事件循环机制发现stack空了，发消息给回调函数队列

18. Callback2被放入stack执行

19. 执行callback2第一行：打印 "我是 callback2"

20. 将callback1的调用从stack中删除

### 总结

- 所有 JS 代码都是在一个线程里执行的，它们在一个stack里同步(synchronous)执行
- 一些我们认为时间长或需要并行执行的内容，其实由运行时环境提供的API来完成，它们是在其他的线程里完成的。
- 加上回调函数和事件循环的机制，让单线程的JS 配上 JS运行时环境 拥有了异步(asynchronous)的能力。
